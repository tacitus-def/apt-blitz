...
| `cache info <HOST> <PATH>` | Show details of a single cached file (URL, perspective host/path, size, MD5/SHA1/SHA256/SHA512 checksums, timestamps, freshness, content type). Checksums are computed on the fly from the stored file. Timestamps are shown in the local timezone in GNU `ls -l` style. Freshness counts down as `HH:MM:SS`. |
